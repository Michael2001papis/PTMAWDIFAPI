...
| **עריכת משימה** | ✎ רק למשתמשים מחוברים – עריכת תיאור ותאריך |
| **מועדפים** | ⭐ רק למשתמשים מחוברים – סימון משימות ואיסוף |
| **התחברות** | רישום (שם, טלפון, סיסמה) והתחברות לחשבון קיים |
| **סימון השלמה** | לחיצה על "הושלם" / "בטל השלמה" |
| **מחיקת משימה** | לחיצה על "מחק" |
| **סינון** | הכל / פעילות / הושלמו / מועדפים |
...
| **שמירה** | שמירה אוטומטית ב-localStorage (משימות ומשתמשים) |
...
1. **התחברות:**  
   לחצו "התחבר / הירשם" – לרישום: שם מלא, טלפון, סיסמה (לפחות 4 תווים).  
   משתמשים רשומים יכנסו דרך טאב "התחברות".
...
3. **עריכה ומועדפים:**  
   רק משתמשים מחוברים רואים כפתורי ✎ (עריכה) ו-⭐ (מועדפים) ליד כל משימה.

4. **סינון משימות:**  
   השתמשו בכפתורים "הכל", "פעילות", "הושלמו" או "מועדפים" להצגת סוג המשימות הרצוי.
...
| **התחברות** | משתמשים נשמרים ב-localStorage – כניסה לחשבון קיים ללא רישום חדש |
