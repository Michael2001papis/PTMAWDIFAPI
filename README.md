...
| **חיפוש** | מציאת משימות לפי טקסט או תאריך + היסטוריית חיפושים |
| **איפוס** | מחיקת כל המשימות |
| **הגדרות** | בחירת צבע (כהה/בהיר/קורל/כחול) וגודל טקסט |
| **אודות** | ℹ הצגת הטכנולוגיות בהתאם לסילבוס |
...
| ✓ | פעולה |
|---|--------|
| ☐ | הרצת `npm i` ו-`npm run build` – וידוא שהכל עובד |
| ☐ | בדיקה בדפדפן – הוספה, סינון, מיון, השלמה, מחיקה |
| ☐ | בדיקה בראשון – התאמה למובייל |
| ☐ | דחיפה ל-GitHub ובדיקה ב-Vercel / GitHub Pages |
| ☐ | קריאת README – הוראות ברורות |
...
| טיפ | תיאור |
|-----|-------|
| **Enter** | לחיצה על Enter בשדה התיאור מוסיפה משימה |
| **פוקוס** | אחרי הוספת משימה – הפוקוס חוזר לשדה הקלט |
| **הודעות** | במקום `alert` – הודעות עדינות שנעלמות אחרי 3 שניות |
| **מצב ריק** | כשאין משימות – מוצגת הודעה ידידותית |
| **נגישות** | `aria-label` ו-`role` לרכיבים חשובים |
| **Escape** | סגירת חלון ההגדרות עם מקש Escape |
| **הגדרות** | צבע וגודל נשמרים אוטומטית |
| **חיפוש** | היסטוריה נשמרת – לחיצה על שדה החיפוש מציגה חיפושים קודמים |
| **הדרכה** | מוצגת למשתמשים חדשים · "הצג הדרכה שוב" בחלון אודות |

---

## 👤 יוצר הפרויקט

**מיכאל פפיסמדוב** · MP · כל הזכויות שמורות © 2026
